...
/**
 * Description of retailsale
 *
 * @author hpt-srieng
 */

class Retail_sale extends CI_Controller{
    //put your code here
    ...
    public function index(){
        ...
        $data['page'] = "retail_sale/list_retail_sale";
        ...
        $data['record_sale_summary']=$this->Base_model->loadToListJoin("SELECT * FROM v_sale_summary WHERE sale_master_type='R' and sale_master_branch_id=".$this->session->userdata("branch_id"));
    ...
    public function addnew(){
        ...
        $data['menu'] = "template/menu";
        $data['footer'] = "template/footer";
        $data['page'] = "retail_sale/frm_retail_sale";
        
        $data['exchange']=$this->Base_model->loadToListJoin('SELECT rate_amount FROM rate');
        $date = new DateTime('now', new DateTimeZone('Asia/Bangkok'));
       //get invoice no
        if($this->Base_model->loadToListJoin("select sale_detail_status from sale_detail where sale_detail_type='R' and sale_detail_status='ACTIVE' and sale_detail_created_by=".$this->session->userdata('user_id'))){
                $invNo=$this->Base_model->loadToListJoin("select max(sale_detail_invoice_no) as invoice_no from sale_detail where sale_detail_created_by=".$this->session->userdata('user_id')." and sale_detail_type='R' and sale_detail_status='ACTIVE' order by sale_detail_id desc limit 1");
        ...
        $data['getlist']=$this->Base_model->loadToListJoin('SELECT sale_detail_id,'
                        ...
                        . '                                         total_with_discount,'
                        . '                                         sale_detail_total'
                        . '                                         FROM v_sale_detail_active where sale_detail_created_by='.$this->session->userdata('user_id')." AND sale_detail_type='R'");
    ...
    public function load_sale_detail_list(){
        ...
        $data['menu'] = "template/menu";
        $data['footer'] = "template/footer";
        $data['page'] = "retail_sale/list_retail_sale_detail";
        ...
        $data['getlist']=$this->Base_model->loadToListJoin("SELECT  sale_detail_id,"
                        ...
                        . "                                         measure_id,"
                        . "                                         sale_detail_total"
                        . "                                         FROM v_sale_detail_finish where sale_detail_type='R' and sale_detail_invoice_no=".$invoice_no);
        ...
        if($this->Base_model->get_data_by("select sale_detail_discount_percent from v_sale_detail_active where sale_detail_discount_percent<>0")){
                $sale_detail_data=array(
                 ...
                 //update data in table sale
                    $this->db->where('sale_detail_type', 'R');
   ...
   public function edit_load(){
        ...
        $data['menu'] = "template/menu";
        $data['footer'] = "template/footer";
        $data['page'] = "retail_sale/frm_retail_sale_u";
        ...
        $get_sale_record=$this->Base_model->loadToListJoin("select item_detail_name,sale_detail_unit_price,sale_detail_unit_price from v_sale_detail_active where sale_detail_id=".$id);
   ...
   public function delete(){
    ...
    public function search(){
        ...
        if($invoiceno == "" && $from =="" && $to =="" && $sale_by =="0" && $recorder =="0" ){$q_invoiceno =''.$q_date =''.$q_sale_by =''.$q_recorder =''.$q_customer ='' .' and sale_master_sell_date= CURDATE()';$data['text_display'] = 'Today Retail Sale Report';}
        ...
        $query = "SELECT * FROM v_sale_summary WHERE sale_master_branch_id=".$this->session->userdata('branch_id')." and sale_master_type = 'R'" .$q_invoiceno.$q_date.$q_recorder.$q_sale_by;
