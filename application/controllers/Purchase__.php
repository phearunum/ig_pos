...
//        }
    ...
    public function addnew_update($id) {
        ...
        $data['record_measure'] = $this->Base_model->loadToListJoin("SELECT * from measure where status=1");
        $data['record_stock_location'] = $this->Base_model->loadToListJoin("select * from stock_location where stock_location_branch_id=(select purchase_branch_id from purchase where purchase_no=$id)");
    ...
    public function save_update_po(){
        $item_id_update=$this->input->post('item_id_update');
        ...
        if($item_id_update==""){
                foreach ($da as $d) {
            if($this->Base_model->check_exists("purchase_no","purchase_detail",array('purchase_no'=>$id,'purchase_detail_item_detail_id'=>$d->item_id))){
                $response["success"] = 1;
                ...
                $data_save_detail=array(
                 ...
                 $this->update_po_master($id);
                 $this->update_credit($id);
             ...
             foreach ($da as $d) {
                ...
                $this->Base_model->updates('purchase_detail',array('purchase_detail_id'=>$po_detail_id),$data_update_detail);
                //update stock with expire info
                $this->Base_model->updates('stock',array('po_detail_id'=>$po_detail_id),array('stock_expire_date'=>$d->ex_date,'stock_alert_date'=>$d->ex_alert));
                $this->update_po_master($id);
...
//        exit();
    ...
    public function response(){        
        ...
        $data['records']=$this->Base_model->loadToListJoin("select * from v_purchase where 1=1 $branch");
    ...
    public function response_search() {    
        ...
        if($sup_id!="" && $sup_id!=null){
            $sup=' and purchase_supplier_id='.(int)$sup_id.' ';
        }
        if($po_id!="" && $po_id!=null){
        ...
        $data['records'] = $this->Base_model->loadToListJoin("select * from v_purchase where 1=1 $branch $s_branch $dates $sup $po");
        $this->load->view("report/report_stock/response",$data);
    }
