...
class Report_sale_summary extends CI_Controller {
    ...
    public function test(){
        $data['records'] = $this->Base_model->loadToListJoin('CALL `p_sale_summary`(" and 1=1")');
...
public function order_list_cash_out(){
        ...
        $query= " AND date_format(sm.sale_master_end_date,'%Y-%m-%d') BETWEEN '$datefrom' AND '$dateto'";
        $transaction_date=" AND date_format(t.transaction_date,'%Y-%m-%d') BETWEEN '$datefrom' AND '$dateto'"; 
        ...
        //Item_Detail
        $Item = $this->Base_model->loadStoreProcedure('call p_sale_category_c("'.$query.'")');

        $Item_void = $this->Base_model->loadStoreProcedure('call p_sale_detail_void("'.$query.'")');
        //$Cashier = $this->Base_model->loadStoreProcedure('call p_cash_register("'.$query.'")');
        $Stock = $this->Base_model->loadStoreProcedure('call p_sale_cut_stock("'.$query.'","'.$branch_id.'","'.$stock_location_id.'")');
        $Summary = $this->Base_model->loadStoreProcedure('call p_sale_summary("'.$query.'")');
        $Summary_Void = $this->Base_model->loadStoreProcedure('call p_sale_summary_void("'.$query.'")');
        ...
        (SELECT sum(sm.sale_master_pax)
          ...
          where sm.sale_master_pax AND sm.sale_master_cash_id=cr.cash_id )  AS pax
            ...
            WHERE  1=1 and sm.sale_master_status in (2) AND sd.sale_detail_status in (1) AND cr.cash_status LIKE 'FINISH' $query
