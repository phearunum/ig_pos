        ...
        <table style="width:100%;" class="table-table">
                    ...
                    ?>
                        <tr>
                            <?php 
//                            if($it->invoice_creator!=''){
                        ?>
                            <td><?php echo $no; ?></td>
                            <td><?php echo $it->invoice_creator; ?></td>
                            <?php // }?>
                            <td>
                                <?php
                                    $invoice_no=$this->Base_model->get_data_by("SELECT invoice_no FROM invoice_finished WHERE master_status IN ('PAID', 'CREDIT','HIDDEN') and detail_status IN ('PAID', 'CREDIT','HIDDEN') and sale_master_sale_by=".$it->sale_master_sale_by.$query."  group by invoice_no desc");
                                    if($invoice_no!=''){
                            ...
                            <td>
                            <?php
                                $grand_total_records=$this->Base_model->get_data_by("SELECT sum(sub_total) as sub_total,sum(total_discount_dollar) as total_discount_dollar,sum(grand_total) as grand_total FROM invoice_finished WHERE master_status IN ('PAID', 'CREDIT','HIDDEN') and detail_status IN ('PAID', 'CREDIT','HIDDEN') and sale_master_sale_by=".$it->sale_master_sale_by.$query." group by invoice_no desc");
                            ...
                            <td>
                            <?php
                                $grand_total_recordes=$this->Base_model->get_data_by("SELECT sum(sub_total) as sub_total,sum(total_discount_dollar) as total_discount_dollar,sum(grand_total) as grand_total FROM invoice_finished WHERE master_status IN ('PAID', 'CREDIT','HIDDEN') and detail_status IN ('PAID', 'CREDIT','HIDDEN') and sale_master_sale_by='".$it->sale_master_sale_by."'".$query." group by invoice_no desc");
                                $total_discount=0;
                                if($grand_total_recordes!=''){
                            ...
                            <td>
                            <?php
                                $grand_total_record=$this->Base_model->get_data_by("SELECT sum(sub_total) as sub_total,sum(total_discount_dollar) as total_discount_dollar,sum(grand_total) as grand_total FROM invoice_finished WHERE master_status IN ('PAID', 'CREDIT','HIDDEN') and detail_status IN ('PAID', 'CREDIT','HIDDEN') and sale_master_sale_by=".$it->sale_master_sale_by.$query." group by invoice_no desc");
                                $sub_total=0;
                                $count_invoice=0;
                        ...
                        <tr class="total" style="background-color:#bbbbbb;">
                    ...
                    ?>
                        <tr class="grand_total" style="background-color: #bbbbbb;">
                        <td colspan="2" style="color:#cc0000; text-align:center;"><?php echo $lbl_summary_grand_total; ?></td>
                        <td><label label style="color:#cc0000;font-weight: bold"><?php echo $grand_count ?></label></td>
                        <td><label style="color:#cc0000;"><?php echo number_format($grand_sub_total,2); ?></label></td>
                        <td><label style="color:#cc0000;"><?php echo number_format($grand_total_discount,2); ?></label></td>
