    ...
    <div id="div">
        ...
        function reprint_shift(){
         ...
         /*           var mytemp=0,qty=0;
                    ...
                    $.each(json.Item,function(l,m){
                        //dis_inv+=parseFloat(m.discount);
                        //alert(m.discount);

                        if(mytemp!=m.invoice_no){
                            var dis_inv=0;
                             $.each(json.Item,function(d,c){
                                if(c.invoice_no==m.invoice_no){
                                    dis_inv+=parseFloat(c.discount);
                                }
                             });
                            mytemp=m.invoice_no;
                            data+='<tr><td colspan="6" class="subleft">'+m.invoice_no+'     (Dis: $'+dis_inv.toFixed(dot_num)+')'+'</td></tr>';
                        }
                    ...
                    data+='<tr><td colspan="4" class="footleft"> Total QTY </td><td colspan="2" class="footcenter">'+qty+'</td></tr>';
