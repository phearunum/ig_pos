      ...
      <div class="modal-body">
            <div class="col-sm-12 col-md-12">
                ...
                <div class="form-group">
                    ...
                    <label><?php echo "Floor Discount" ?></label>
                    <input class="form-control" type="text" name="txt_floor_dis" id="txt_floor_dis" placeholder="Floor Discount(%)">   
                    <label><?php echo "Is Delivery" ?></label>
                    <input type="checkbox"  name="ch_floor_del" id="ch_floor_del" value="0"> 
...
<div style='overflow: auto; height:76px !important; width: 100%; overflow-x:  hidden;'>
  ...
  <script type="text/javascript">
      $(document).ready(function(){
            load_data();
            $('#txt_floor_dis').attr('disabled',true);
      ...
      $('#ch_floor_del').change(function() {
          if($(this).is(":checked")) {
            $('#txt_floor_dis').attr('disabled',false);
             $(this).val(1);
          }else{
            $('#txt_floor_dis').val(0);
            $('#txt_floor_dis').attr('disabled',true);
      ...
      $('#btn_save_floor').on('click',function(){
            ...
            var dl=$('#ch_floor_del').val();
            var dis=$('#txt_floor_dis').val();
            if(check_text_val_empty('txt_floor_name')==false){
              ...
              $.ajax({
                  ...
                  type:'post',
                  data:{'id':b_id,'name':name,'f_id':id,'dis':dis,'delivery':dl},
      ...
      function edit_floor(id,name,branch_id,dis,is_delivery){
        clear_floor();
        $('#model_floor').modal('show');
        $('#txt_floor_name').val(name);
        $('#txt_floor_id').val(id);
        $('#txt_branch_id').val(branch_id);
        $('#txt_floor_dis').val(dis);
        if(is_delivery==1){
          $('#ch_floor_del').prop('checked',true);
          $('#ch_floor_del').val(1);
          $('#txt_floor_dis').prop('disabled',false);
        }
        else{
          $('#ch_floor_del').prop('checked',false);
          $('#ch_floor_del').val(0);
           $('#txt_floor_dis').prop('disabled',true);
        }
                ...
                $.each(json.layout,function(i,v){
                    ...
                    header+='<li class="'+active_class+'"><a  href="#'+v.branch_id+'" data-toggle="tab">'+v.branch_name+'</a></li>';
                        ...
                        $.each(v.floor,function(ii,vv){
                            floor_all+='<div class="floor" onClick="load_table('+v.branch_id+','+vv.floor_id+')"><span>'+vv.floor_name+'</span><br><a <?php if(!$permission_edit=="disabled"){echo "onClick=\"edit_floor('+vv.floor_id+','+\"'\"+vv.floor_name+\"'\"+','+v.branch_id+','+vv.dis+','+vv.is_delivery+')\" ";} echo $edit_link; ?> ><i style="color: white;" class="fa fa-edit action-floor'+v.branch_id+' hidden" ></i></a>&nbsp;<a <?php if(!$permission_edit=="disabled"){echo "onClick=\"del_floor('+vv.floor_id+')\"";} echo $delete_link; ?> ><i style="color: white;" class="fa fa-trash-o action-floor'+v.branch_id+' hidden" ></i></a></div>';
                        }); 
